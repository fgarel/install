...
function getMap(){
    ...
    if(nmbr=="0"){
        ...
        var paramsLayer = "eclairage";
				//alert("paramsLayer = " + paramsLayer);
        ...
        $.ajax({
            type: "POST",
            url: "http://djangoqgis.mairie.fr/qgis/getMap/",
            data: {
							content: "layers=" + paramsLayer + "&orientation=" + paramOrientation + "&format=" + paramFormat+ "&geojson=" + json,
...
function initMap(){
	...
	var projectionCC46 = new ol.proj.Projection({
	  ...
	  extent: [1372000.0, 5207000.0, 1403020.0, 5238000.0],
	  units: 'm'
	  });
